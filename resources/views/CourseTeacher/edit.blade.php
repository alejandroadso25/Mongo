    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('course-teachers.update', $courseTeacher->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select id="teacher_id" name="teacher_id" class="form-select @error('teacher_id') is-invalid @enderror" required>
                                ...
                                @foreach ($teachers as $teacher)
                                    <option value="{{ $teacher->id }}" {{ old('teacher_id', $courseTeacher->teacher_id) == $teacher->id ? 'selected' : '' }}>{{ $teacher->name }}</option>
                                @endforeach
                            </select>
                            @error('teacher_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <a href="{{ route('course-teachers.show', $courseTeacher->id) }}" class="btn btn-secondary">Cancelar</a>
                        </div>
                    </form>
                </div>
            </div>
